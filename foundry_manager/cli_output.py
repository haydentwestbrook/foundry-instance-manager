...
def format_instance_table(instances: List[dict]) -> Table:
    ...
    for instance in instances:
        ...
        link = f"http://localhost:{instance['port']}" if status == "[green]running[/green]" else "-"
...
def format_versions_table(versions: List[dict]) -> Table:
    ...
    for version in versions:
        ...
        if "stable" in version_str.lower():
            version_type = "stable"
        elif "dev" in version_str.lower() or "alpha" in version_str.lower() or "beta" in version_str.lower():
...
def print_versions_table(versions: List[str]) -> None:
